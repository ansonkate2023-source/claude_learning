# Claude 學習知識庫 — 每日總覽 2026-04-06

## 學習資源分類

| 分類 | 篇數 | 重點內容 |
|------|------|----------|
| [報告撰寫方法論](./report-writing/methodology.md) | 1 | **五階段流程：** 定義→研究→大綱→撰寫→審查 |
| [Skill 開發指南](./skill-development/summary.md) | 3 | 官方最佳實踐 + 測試框架 + 開發方法論 |
| [Claude Code](./claude-code/summary.md) | 20 | MagicDocs 移除分析、System Prompt 解析、社群 Skills、LLM Router |
| [Prompt Engineering](./prompt-engineering/summary.md) | 5 | 官方最佳實踐、15 種模式、FORGE 方法、每日 5 技巧 |
| [生產力案例](./productivity/summary.md) | 8 | Opus 9 大優勢、終端機日常化、用量限制應對 |
| [官方最佳實踐](./best-practices/summary.md) | 4 | 輸出控制、Agent 系統、13 語言效能基準測試 |
| [新功能與更新](./features/summary.md) | 8 | Computer Use、Mythos 模型、Adaptive Thinking |
| [Claude 新聞](./Claude/summary.md) | 17 | 源碼洩露事件、市場動態、政治活動 |

**總計：** 62 篇學習資源 + 17 篇新聞

## 本次更新新增（第 6 輪收集）

### 新增 1 篇：
- **MagicDocs 移除** — Claude Code 內部架構演進分析，舊的自動文件功能被 skill 框架取代（translunar.io）

## 今日重點學習

1. **Prompt 寫法：** 說「要做什麼」比「不要做什麼」更有效
2. **XML 標籤：** 結構化 prompt 能大幅減少誤解
3. **Claude Code Skills：** 社群已開發大量實用 skill（Grug、Crabby、Render 等）
4. **LLM Router：** 用 MCP server 將簡單任務路由到便宜模型，省錢
5. **System Prompt：** 理解 Claude Code 如何組裝 prompt，寫出更好的 CLAUDE.md
6. **Adaptive Thinking：** Claude 4.6 自動調整思考深度，用 `effort` 控制

## 資料來源
- Anthropic 官方文件
- TechCrunch / Tom's Guide / WIRED / eWeek / XDA
- Hacker News / Google News
- Blockchain Council
- 社群部落格（dbreunig.com, ai.georgeliu.com）
