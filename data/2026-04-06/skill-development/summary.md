...
## 九-C、Skill 優化實戰經驗（社群案例）

來源：aaroneaton.com「We Built a Skills Optimizer」

一個團隊建了 skill-evolution 系統，自動測試與優化 Claude Code skills。核心發現：

### 1. 壓縮 prompt 會摧毀品質
縮短 skill 指令四次，品質從 89.4% 降到 59%。「壓縮不只是減少詞彙——它摧毀了模型推理邊界情況的支架。」

**教訓：** 不要為了省 token 砍 skill 指令。詳細步驟是必要的。

### 2. 測試環境會污染結果
他們的評分標準意外存在 CLAUDE.md 中，Claude 讀到了「答案」。100% 的分數測的是「skill + 環境洩漏」，非 skill 本身。

**教訓：** 用 `claude --bare -p` + 臨時目錄隔離測試環境。

### 3. 單次測試不代表真實品質
同一 prompt 因標點不同得分不同。可靠結論需要每個測試至少跑 100 次。

**教訓：** 我們的測試框架一次跑一次，結果只能當參考，不能當定論。

### 4. 70% 的測試在測模型通用能力
真正有價值的測試是「Freshman 精確度」——測 skill 帶來的**特定行為**，而非 Claude 本來就會做的事。

**教訓：** 寫測試案例時，問自己「不用這個 skill，Claude 也會這樣做嗎？」如果是，這個測試沒有價值。

### 5. 跨模型驗證很重要
要確認 Sonnet + skill 是否超越裸 Opus。如果不超越，skill 沒有存在價值。

### 實踐建議總結

| 建議 | 行動 |
|------|------|
| 不壓縮 skill | 保持詳細步驟，不為省 token 砍指令 |
| 隔離測試 | `claude --bare -p` + 臨時目錄 |
| 多次採樣 | 單次結果當謠言，100+ 次才可靠 |
| 測 skill 價值 | 只測 skill 帶來的特定行為 |
| 跨模型驗證 | Sonnet+skill vs 裸 Opus |
| 模型升級時重測 | 新版本可能破壞現有 skill |
...
### v5 關鍵修正（從 v4 review 發現的問題）

| 問題 | v4 | v5 |
|------|----|----|
| grep 特殊字元 | `grep -qi` 把 `$` 當 regex | `grep -F` fixed string |
| 跨行 excludes | `---\nname:` 永遠通過 | 拆成多個單行條件 |
| 中文關鍵詞提取 | 「用 bash 提取」但做不到 | 砍掉，只比長度/行數 |
| SKILL.md 太長 | 246 行 | ~120 行，只留執行指令 |
| 子代理兩個任務 | 「執行 skill + 寫檔 + 回報 JSON」 | 只要求「執行 + 寫檔」 |

### 已知限制（誠實聲明）

1. **子代理可能不寫檔** — 檢查檔案存在作為 fallback
2. **快照只比長度和行數** — 只偵測大幅變化，不偵測微妙退步
3. **非確定性輸出** — 同一測試可能時過時不過，這是正常的
4. **每個測試一個子代理** — 建議總數 ≤ 12
5. **子代理共享檔案系統** — side_effects 有 teardown 但非完全隔離
