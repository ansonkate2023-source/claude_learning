...
## 十一、MagicDocs 被移除 — Claude Code 內部架構演進（新）

來源：translunar.io

### MagicDocs 是什麼？
Anthropic 內部工具，用來在 Claude Code 回應後自動啟動子代理更新架構文件。強調「簡潔、高信號、當前狀態」的文件哲學。

### 為什麼被移除？
1. **設計過時** — 使用過時的代理提示架構，缺乏測試機制
2. **功能重疊** — 新的技能框架和記憶系統已能實現類似功能
3. **無反饋迴圈** — 無法驗證生成的文件品質
4. 134 天內從未被修改，是最少更新的系統提示之一

### 學習重點
- Claude Code 內部持續淘汰舊功能、引入新架構
- **文件自動化的正確方式**：需要反饋迴圈和品質驗證
- 新的 skill 框架 + memory 系統是 Anthropic 目前偏好的方向
